...
const addHotel_to_Itinerary = asyncHandler(async (req, res) => {
  ...
  const newHotel = await Itinerary.findByIdAndUpdate(itineraryId, {
    $push: {
      hotels: {
        name,
        description,
  ...
  return res
    .status(201)
    .json(new ApiResponse(200, {}, "Hotel added Successfully"));
});
...
const getitinerary_by_user = asyncHandler(async (req, res) => {
  const itinerary = await Itinerary.find({ userId: req?.user_id }).populate(
    "destinations"
  );
  if (!itinerary) {
    throw new ApiError(404, "Itinerary not found.");
...
const delete_Itinerary = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, {}, "Itinerary deleted successfully."));
});
